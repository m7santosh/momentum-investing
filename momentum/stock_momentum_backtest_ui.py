"""Tkinter backtest UI for stock momentum rankers."""
...
from backtest.stock.backtest_stock_momentum import (  # noqa: E402
    REBALANCE_ALIASES,
    REBALANCE_LABELS,
    STRATEGY_KEYS,
    STRATEGY_LABELS,
    StockMomentumBacktestConfig,
    StockMomentumBacktestEngine,
...
def _apply_strategy_defaults(
    ...
    rebalance_var.set(REBALANCE_LABELS.get(defaults["rebalance_period"], "Biweekly"))


def open_stock_momentum_backtest(
    parent: tk.Misc,
    *,
    initial_strategy: str = "quality_momentum_rs_lv",
    ...
    win.title("Stock Momentum Backtest")
    win.geometry("1120x800")
    win.minsize(920, 580)
    win.lift()

    engine: StockMomentumBacktestEngine | None = None
    _busy = False
    ...
    rebalance_var = tk.StringVar(value="Biweekly")
    ttk.Combobox(
        params,
    ...
    portfolio_var = tk.IntVar(value=20)
    ttk.Spinbox(params, from_=1, to=50, width=4, textvariable=portfolio_var).grid(
        row=0, column=9, **_pad
    )
    ...
    exit_rank_var = tk.IntVar(value=30)
    exit_rank_spin = ttk.Spinbox(
        params, from_=1, to=100, width=4, textvariable=exit_rank_var
    )
    ...
    stop_loss_pct_var = tk.DoubleVar(value=10.0)
    ...
    mode_row.grid(row=1, column=9, columnspan=3, sticky="w", pady=(6, 0))
    ...
    def _rebalance_key() -> str:
        ...
        for key, text in REBALANCE_LABELS.items():
            if text.lower() == label:
                return key
        return REBALANCE_ALIASES.get(label, "biweekly")  # type: ignore[name-defined]
    ...
    def _ui_config() -> StockMomentumBacktestConfig:
        return build_config_from_ui(
            ...
            stop_loss_pct=_parse_stop_loss_pct() if exit_stop_loss_var.get() else 10.0,
        )
    ...
    def _config_needs_reload(old: StockMomentumBacktestConfig, new: StockMomentumBacktestConfig) -> bool:
        return (
    ...
    def _on_pick_select(_event=None) -> None:
        sel = pick_tree.selection()
        if not sel:
            return
        vals = pick_tree.item(sel[0], "values")
        if len(vals) >= 7:
            status_var.set(f"{vals[0]}: {vals[6]}")

    pick_tree.bind("<<TreeviewSelect>>", _on_pick_select)
    ...
    def _on_load() -> None:
        ...
        def worker() -> None:
            ...
            eng: StockMomentumBacktestEngine | None = None
            try:
                eng = StockMomentumBacktestEngine(ui_cfg)
                eng.load_data()
    ...
    def _on_load_done(eng: StockMomentumBacktestEngine | None, err: Exception | None) -> None:
        nonlocal engine, _load_run_all_after
        _set_busy(False)
        if err is not None:
...
def main() -> int:
    root = tk.Tk()
    root.withdraw()
    open_stock_momentum_backtest(root)
    root.mainloop()
    return 0
