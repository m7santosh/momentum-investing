"""
Quality ~130 — RS vs ^CRSLDX with rebalance/state; no low-volatility filter.

Universe: universes/quality.py
Filters: 200 EMA, 52w-high proximity, ADTV > 5 Cr (low-vol quantile omitted).
Rank: blended abs + RS on 3M/6M/9M; calendar rebalance + portfolio state (same machinery as _lv).

vs other stock scripts:
- quality_momentum_rs_lv.py — same universe; adds low-vol quantile before ranking.
- quality_momentum_rs.py — same universe/rank; daily scan only, no rebalance state.
- quality_momentum_rs_lv_list.py — includes low-vol; snapshot list only, no state.
...
def _effective_low_vol_max_quantile() -> float:
    """Sentinel for rebalance context matching (no cross-sectional LV cap in this script)."""
    return -1.0
