"""
Nifty 500 — RS vs ^CRSLDX + low-volatility; calendar rebalance with portfolio state.

Universe: universes/n500.py
...
CLI/env: session date, rebalance period; writes state + archive JSON (same engine as quality _lv).

vs other stock scripts:
- quality_momentum_rs_lv.py — Quality ~130 universe; otherwise same rebalance/LV logic.
- quality_momentum_rs.py — Quality ~130; no low-vol; daily scan only.
...
# --- Ticker universe: momentum/stock/universes/n500.py (edit tickers there)
from momentum.stock.universes.n500 import tickers
