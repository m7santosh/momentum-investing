...
from momentum.etf.universes.us import (  # noqa: E402
...
STRATEGY_LABELS: dict[str, str] = {
    ...
    "momentum_us_etfs_recommended": "Top N Recommended",
}
...
def strategy_defaults(strategy_key: str) -> dict[str, Any]:
    ...
    if strategy_key == "momentum_us_etfs_recommended":
...
def _slot_quantity(slot_budget: float, unit_price: float | None) -> float | None:
    if unit_price is None or unit_price <= 0 or slot_budget <= 0:
        return None
    return slot_budget / unit_price
...
class EtfMomentumBacktestEngine:
    ...
    def _build_period_position_rows(
        ...
        *,
        ...
        park_sym = park_slot_etf.strip().upper()
        rank_map = (
    ...
    def step_period(self) -> dict | None:
        ...
        rank_daily_close = (
            self._daily_close_excel()
            if cfg.strategy_key == "momentum_us_etfs_recommended"
            else None
        )
        ...
        if cfg.park_empty_slots and holdings and not go_cash:
            park_etf = cfg.park_slot_etf.strip().upper()
...
def build_config_from_ui(
    *,
    ...
    park_etf = (park_slot_etf or defaults["park_slot_etf"]).strip().upper()
    if park_etf not in PARK_SLOT_ETFS:
        park_etf = DEFAULT_PARK_SLOT_ETF
...
def run_backtest_cli(
    *,
    ...
    cfg = build_config_from_ui(
        ...
        park_empty_slots=park_empty_slots,
        park_slot_etf=park_slot_etf,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-park-empty-slots",
        action="store_true",
        help="Disable parking empty slots in SGOV/BIL (default: on)",
    )
    parser.add_argument(
        "--park-slot-etf",
        default=None,
        help="Park ETF for empty slots (SGOV or BIL; default SGOV)",
    )
    ...
    metrics = run_backtest_cli(
        ...
        park_empty_slots=not args.no_park_empty_slots,
        park_slot_etf=args.park_slot_etf,
    )
    if not metrics:
        print("No results.")
        return 1
