"""
Walk-forward backtest for the ETF RS momentum strategy with 9 EMA exit guard.

Weekly rebalance:
    Re-runs the ranking pipeline (trend filters, abs-return ranks,
    RS ranks, Blended_Rank) using only data up to that date.

Entry / Exit rules:
    - Entry : ETF must rank <= TOP_N to enter the portfolio.
    - Hold  : rank <= WORST_RANK_HELD -> keep unconditionally.
    - Watch : rank > WORST_RANK_HELD -> put on daily "9 EMA exit watch".
              If Close drops below 9 EMA on any day (including mid-week),
              the ETF is exited at that day's close. If it stays above
              9 EMA, it survives until the next rebalance re-evaluation.
    - The portfolio is capped at TOP_N equal-weight slots.
...
Configurable:
    ...
    WORST_RANK_HELD                - rank threshold for 9-EMA watch
                                     (>= TOP_N).
    USE_REGIME_FILTER              - go 100% cash when Trend_Down.
    INITIAL_CAPITAL                - starting value.

Examples:
    python backtest/etf/backtest_momentum_rs_etfs_9ema.py --top-n 5 --worst-rank 5
    python backtest/etf/backtest_momentum_rs_etfs_9ema.py --top-n 5 --worst-rank 10
"""
...
BACKTEST_OUT_DIR = FINAL_RESULT_ETF_DIR / "backtest_9ema"
...
def _download_all(tickers: list[str], start: str, end: str) -> dict[str, dict[str, pd.Series]]:
    """Bulk download Adj Close + Close for every ticker."""
...
def _rank_at_date(
    etf_data: dict[str, dict[str, pd.Series]],
    bench_adj: pd.Series,
    as_of: pd.Timestamp,
) -> pd.DataFrame:
    rows: list[dict] = []
    ...
    for sym, series_dict in etf_data.items():
        adj = series_dict["adj"].loc[:as_of]
        if len(adj) < LB_52W:
            continue
...
def _select_holdings(
    ...
    Hold rule  : rank <= worst_rank_held -> keep unconditionally.
    Watch rule : rank > worst_rank_held -> keep on "9 EMA exit watch";
                 exit mid-week when Close < 9 EMA.
                 If already below 9 EMA at rebalance, exit now.

    Returns (holdings, on_watch).
    """
    if ranked_df.empty:
        return [], set()
    ...
    for sym in prev_holdings:
        if sym not in rank_map:
            ...
            continue
        if rank_map[sym] <= worst_rank_held:
            safe.append(sym)
        else:
            watch_candidates.append(sym)

    on_watch: set[str] = set()
    for sym in watch_candidates:
        if sym not in etf_data:
            continue
        close = etf_data[sym]["close"].loc[:as_of]
        if len(close) < ETF_EMA_9:
            continue
        ema9 = float(close.ewm(span=ETF_EMA_9, adjust=False).mean().iloc[-1])
        if float(close.iloc[-1]) >= ema9:
            on_watch.add(sym)

    retained = safe + [s for s in watch_candidates if s in on_watch]
    ...
    retained.sort(key=lambda s: rank_map.get(s, 9999))
    holdings = retained + new_entries
    holdings = holdings[:top_n]

    on_watch = on_watch & set(holdings)
    return holdings, on_watch
...
def run_backtest(
    ...
    bench_adj = bench_data[benchmark_ticker]["adj"]
    ...
    for i, rebal_date in enumerate(weekly_dates[:-1]):
        ...
        if go_cash:
            ...
            holdings, on_watch = _select_holdings(
                ranked_df, prev_holdings, top_n, worst_rank_held, etf_data, rebal_date,
            )

        intraweek_days = all_dates[(all_dates > rebal_date) & (all_dates <= next_date)]
        per_slot: list[dict] = []
        mid_week_exits_week = 0

        for sym in holdings:
            adj_s = etf_data[sym]["adj"]
            entry_slice = adj_s.loc[:rebal_date]
            if len(entry_slice) == 0:
                per_slot.append({"sym": sym, "ret": 0.0, "exited_early": False})
                continue
            p_entry = float(entry_slice.iloc[-1])

            if sym not in on_watch:
                exit_slice = adj_s.loc[:next_date]
                p_exit = float(exit_slice.iloc[-1]) if len(exit_slice) > 0 else p_entry
                per_slot.append({
                    "sym": sym,
                    "ret": (p_exit / p_entry - 1) if p_entry > 0 else 0.0,
                    "exited_early": False,
                })
                continue

            close_s = etf_data[sym]["close"]
            exited_early = False
            for day in intraweek_days:
                cs = close_s.loc[:day]
                if len(cs) < ETF_EMA_9:
                    continue
                ema9 = float(cs.ewm(span=ETF_EMA_9, adjust=False).mean().iloc[-1])
                if float(cs.iloc[-1]) < ema9:
                    p_exit = float(adj_s.loc[:day].iloc[-1])
                    per_slot.append({
                        "sym": sym,
                        "ret": (p_exit / p_entry - 1) if p_entry > 0 else 0.0,
                        "exited_early": True,
        ...
        port_ret = float(np.mean(slot_returns))
        ...
        mid_week_exits_total += mid_week_exits_week

        end_of_week_holdings = [
            s["sym"] for s in per_slot if not s.get("exited_early", False)
        ]
        ...
        records.append({
            ...
            "On_Watch": ", ".join([s.replace(".NS", "") for s in on_watch]) or "-",
            "Mid_Week_Exits": mid_week_exits_week,
        ...
        prev_holdings = end_of_week_holdings
    ...
    print(f"  9-EMA mid-week exits: {mid_week_exits_total} over {len(df_trades)} weeks")
...
def _compute_metrics(df: pd.DataFrame, capital: float, bench_name: str, top_n: int = TOP_N, worst_rank_held: int = WORST_RANK_HELD, regime_filter: bool = False, total_trades: int = 0) -> dict:
    ...
    mid_week_exits = int(df["Mid_Week_Exits"].sum()) if "Mid_Week_Exits" in df.columns else 0
    ...
    return {
        ...
        "Weeks": n_weeks,
        "Mid_Week_9EMA_Exits": mid_week_exits,
...
def _print_metrics(m: dict) -> None:
    print("\n" + "=" * 55)
    print("  BACKTEST RESULTS [9-EMA EXIT]")
    print("=" * 55)
    fmt = [
        ...
        ("9-EMA Mid-Week Exits", m.get("Mid_Week_9EMA_Exits", 0)),
        ("",                     ""),
...
def _write_excel(df_trades: pd.DataFrame, metrics: dict, bench_name: str) -> None:
    out_path = BACKTEST_OUT_DIR / f"backtest_rs_etfs_9ema_{bench_name.lower()}.xlsx"
    if out_path.exists():
        try:
...
def _plot_equity(df_trades: pd.DataFrame, metrics: dict, bench_name: str, top_n: int = TOP_N, worst_rank: int = WORST_RANK_HELD) -> None:
    ...
    ax1 = axes[0]
    ax1.plot(dates, strat_vals, label="Strategy (9-EMA exit)", linewidth=1.5, color="#1f77b4")
    ...
    ax1.set_title(
        f"ETF RS Momentum [9-EMA Exit]  |  Top {top_n} (hold to {worst_rank})  |  "
    ...
    plt.tight_layout()
    png_path = BACKTEST_OUT_DIR / f"backtest_rs_etfs_9ema_{bench_name.lower()}.png"
...
RUN_LOG_PATH = BACKTEST_OUT_DIR / "backtest_9ema_run_log.csv"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Backtest ETF RS Momentum Strategy [9-EMA Exit]")
    ...
    parser.add_argument("--worst-rank", type=int, default=WORST_RANK_HELD,
                        help="Exit buffer: existing holding goes on 9-EMA watch above this rank (>= top-n)")
